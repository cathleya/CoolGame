...
class CGChatRevCell: UITableViewCell {
    ...
    var messageInfo:[String:Any]?{
        ...
        set{
            ...
            msgBody.text = msg
            msgBody.frame = CGRect(x: 50, y: 10, width: mWidth, height: mHeight)
            
            bglayer.removeFromSuperlayer()
            layer.addSublayer(self.contour(45, 5, mWidth, mHeight+12))
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        portrait = UIImageView(frame: CGRect(x: 5, y: 5, width: 30, height: 30))
        portrait.backgroundColor = UIColor.magenta
        portrait.image = UIImage(named: "reciever.png")
        addSubview(portrait)
        
        
        msgBody = UILabel(frame: CGRect(x: 50, y: 10, width: SCREEN_WIDTH-100, height: 25))
        msgBody.font = UIFont.systemFont(ofSize: 12)
        msgBody.lineBreakMode = .byWordWrapping
    ...
    func contour(_ xMin:CGFloat,_ yMin:CGFloat,_ cWidth:CGFloat,_ cHight:CGFloat) -> CAShapeLayer{
        ...
        let arrowPointX = xMin-5.0
        let arrowPointY = yMin + 10.0
        
        
        bezier.removeAllPoints()
        
        bezier.move(to: CGPoint(x: xMin, y: yMin+5)) 
        ...
        bezier.addCurve(to: CGPoint(x: xMin, y: arrowPointY+5),
                        controlPoint1: CGPoint(x: xMin, y: arrowPointY+2),
                        controlPoint2: CGPoint(x: xMin, y:arrowPointY+5))
        
        bezier.addLine(to: CGPoint(x: xMin, y: yMax-5))
        bezier.addCurve(to: CGPoint(x: xMin+5, y: yMax),
                        controlPoint1: CGPoint(x: xMin, y: yMax),
                        controlPoint2: CGPoint(x: xMin, y:yMax))
        
        bezier.addLine(to: CGPoint(x: xMax-5, y: yMax))
        bezier.addCurve(to: CGPoint(x: xMax, y: yMax-5),
                        controlPoint1: CGPoint(x: xMax, y: yMax),
                        controlPoint2: CGPoint(x: xMax, y:yMax))
        
        bezier.addLine(to: CGPoint(x: xMax, y: yMin+5))
        bezier.addCurve(to: CGPoint(x: xMax-5, y: yMin),
                        controlPoint1: CGPoint(x: xMax, y: yMin),
                        controlPoint2: CGPoint(x: xMax, y:yMin))
        
        bezier.addLine(to: CGPoint(x: xMin+5, y: yMin))
        bezier.addCurve(to: CGPoint(x: xMin, y: yMin+5),
        ...
        bglayer.fillColor = CG_RECMSGBGCOLOR.cgColor
